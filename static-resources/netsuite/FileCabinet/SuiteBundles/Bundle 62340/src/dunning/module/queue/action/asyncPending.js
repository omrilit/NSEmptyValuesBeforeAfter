...
function (deployments, RequestQueueEntry, C) {
  ...
  return function (internalIds, letterTypeId) {
    const data = {};
    data[C.QUEUE_MODULE.ASYNC_PENDING.PARAM.IDS] = JSON.stringify(internalIds);
    data[C.QUEUE_MODULE.ASYNC_PENDING.PARAM.TYPE] = letterTypeId;

    const scripts = deployments.search(C.QUEUE_MODULE.ASYNC_PENDING.SCRIPT);
