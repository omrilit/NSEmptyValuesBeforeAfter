...
define([
	'../adapter/itr_local_adapter_file',
	'../adapter/itr_local_adapter_runtime',
	'../adapter/itr_local_adapter_search'
...
function (file, runtime, search) {
    ...
    function _GetTranslation() {
        ...
        if(!this.locale){
            this.locale = runtime.getCurrentUser().getPreference(LANG);
        }
        
        fileId = _GetFileId("itr_en_US.json");
        strTranslations = file.load({id: fileId});
        translation = JSON.parse(strTranslations.getContents());

        if (this.locale.substring(0,2) !== "en") {
            try {
            	fileId = _GetFileId("itr_"+this.locale.replace("-","_")+".json");
            	if(fileId){
            		strTranslations = file.load({id: fileId});
