...
function ep_PageInit() {
    ...
    if (paymentType == 'eft') {
		...
		alert('Error - payment type is not set.');
        return;	
    }

    // set the processing date
    if (!nlapiGetFieldValue('custpage_2663_process_date')) {
        nlapiSetFieldValue('custpage_2663_process_date', nlapiDateToString(new Date()));
    }
	...
	if (!document.forms['main_form'].onsubmit || document.forms['main_form'].onsubmit()) {
		...
		if (bankAccount && (approvalRoutingEnabled || batchId)) {
			if (!nlapiGetFieldValue('custpage_2663_payment_ref')) {
				alert('Please enter value(s) for: EFT file reference note');
				return false;
			}
		    	...
		    	if (approvalRoutingId) {
	            	...
	            	if (paymentLimit && approvalAmount > paymentLimit) {
	            		if (approvalLevel == APPROVAL_LEVEL3) {
	            			alert('Payment limit of Highest level approver exceeded. Please revisit your Approval Routing settings.');
		    		...
		    		alert('Incomplete approval routing setup!');
		    	}	
			}
		}
        else if (upsertBatch(bankAccount, batchId)) {
		...
		if (tranKeys.length > 0 || summarized) {
			...
			if (!summarized) {
				...
				for (var i = 0; i < numOfBatches; i++) {
			        ...
			        if (error) {
			             alert(['An error occurred while calling the restlet:', error.code, error.message].join(' '));
			             res = false;
			             break;
			        }
			        if (responsebody['update'] != 'complete') {
						alert(['Batch update did not complete:', responsebody['update']].join(' '));
						res = false;
						break;
			        }
	...
	if (batchId) {
		try {
			...
			for (var i = 0; i < numOfBatches; i++) {
		        ...
		        if (responsebody['update'] != 'complete') {
					alert(['Batch rejection did not complete:', responsebody['update']].join(' '));
					res = false;
					break;
		        }
		        progress = (i + 1) / numOfBatches;
...
function adminSubmitField(recType, recId, fldName, fldValue, doSourcing) {
    try {
        ...
        alert('Error during adminSubmitField of field ' + fldName + ' under record ' + recType + ' with ID ' + recId);        
    }
        ...
        for (var i = 1; i <= numPages; i++) {
            ...
            if (nlapiGetFieldValue('custpage_2663_exchange_rates')) {
                try {
                    ...
                    alert('Unable to parse exchange rate string: ' + nlapiGetFieldValue('custpage_2663_exchange_rates') + '. Setting default rate to 1.');
                }
            }
    ...
    function setChequeNumbers() {
        ...
        if (firstCheckNo && firstCheckNoHidden) {
            if (parseInt(firstCheckNo, 10) != parseInt(firstCheckNoHidden, 10)) {
                alert('First Cheque Number field is set to Check # with smallest value');
        ...
        if (lastCheckNo && lastCheckNoHidden) {
            if (parseInt(lastCheckNo, 10) != parseInt(lastCheckNoHidden, 10)) {
                alert('Last Cheque Number field is set to Check # with highest value');
		...
		for (var i in markedLines) {
			for (var j in markedLines[i]) {
                ...
                if ( hasEmail(markedLineValue) ){
					...
					alert('Please specify an email address to send to. Email Address is required.');
					return false;                
                }
			}
		}
...
psg_ep.PaymentSelectionClient = function(interfaceObj) {
    if (!interfaceObj) {
        throw nlapiCreateError('EP_INTERFACE_OBJ_NULL', 'Cannot create object without interface for Client', true);
    }
...
psg_ep.PaymentSelectionSublistClient = function(uiObj) {
    if (!uiObj) {
        throw nlapiCreateError('EP_UI_OBJ_NULL', 'Cannot create sublist object without ui object', true);
    }
    ...
    function fieldChanged(type, name, lineNum, currFlag, otherProps) {
        ...
        if (name == 'custpage_pay' || name == 'custpage_discamount' || name == 'custpage_payment' || (otherProps && isInArray(name, otherProps))) {
            ...
            if ((name == 'custpage_payment' && Math.abs(payAmount) > 0 && toPay == 'F' && maxLinesSel == paymentLines) ||
                ...
                alert('Maximum number of transactions that can be marked is: ' + nlapiGetFieldValue('custpage_2663_max_lines_sel'));
                payAmount = 0;
                    ...
                    if (isInArray(tranType,creditRecTypes)) {
                        ...
                        if( validDateForDiscount && isInArray(tranType, validTranTypeList) && origDiscAmt){
                            ...
                            if(payAmount && payAmount > 0){
                            	...
                            	if(discAmt < 0){
    		                    	alert('The payment and discount amount, when combined, must not exceed the remaining ' + tranType + ' amount.');
                        	...
                        	alert('The payment amount must not exceed the remaining ' + tranType + ' amount.');
                        	if(nlapiGetLineItemValue('custpage_2663_sublist', 'custpage_pay', lineNum) == 'T'){
	                    ...
	                    if(isInArray(tranType, validTranTypeList) && origDiscAmt && ((dateToBeProcessed <= nlapiStringToDate(discDate) && discDate) || (origDiscAmt && !discDate))){
	                    	if(discAmt && discAmt >= 0){
	                    		if(discAmt > maxDiscAmt){
		                    		alert('The discount amount must not exceed the calculated maximum discount amount.');
		                        	nlapiSetLineItemValue('custpage_2663_sublist', 'custpage_discamount', lineNum, origDiscAmt ? nlapiFormatCurrency(origDiscAmt) : '');
		                    	}
	                    	} else if (discAmt){
		                    	alert('The discount amount must be greater than zero.');
		                    	nlapiSetLineItemValue('custpage_2663_sublist', 'custpage_discamount', lineNum, origDiscAmt ? nlapiFormatCurrency(origDiscAmt) : '');
	                    	}
	                    	...
	                    	if(!isInArray(tranType, validTranTypeList)){
	                    		alert("Discounts can be applied to vendorbill or invoice transactions only.");
	                    	} else if(!origDiscAmt){
	                    		alert("Discounts cannot be applied to bills with no existing terms.");
	                    	} else if(dateToBeProcessed > nlapiStringToDate(discDate) && discDate){
	                    		alert("Discounts cannot be applied after the discount period.");
	                    	}
	                    	nlapiSetLineItemValue('custpage_2663_sublist', 'custpage_discamount', lineNum, '');
	                    }
                    }
...
psg_ep.SublistClient = function(uiObj) {
    ...
    if (!nlapiGetFieldValue('custpage_sublist_ep_name')) {
        throw nlapiCreateError('EP_SUBLIST_ID_NOT_SET', 'The sublist id is not set', true);
    }
    ...
    function checkSelectedLines(paymentType, batchFlag) {
        ...
        if (paymentType == 'eft' || paymentType == 'dd') {
            ...
            if (nlapiGetFieldValue('custpage_2663_total_amount') <= 0 && nlapiGetFieldValue('custpage_2663_payment_lines') == 0) {
                if (batchFlag == false) {
                    alert('Please enter payment for at least 1 transaction.');
                }
                else  {
                    alert('There are no transactions for processing.');
                }
                result = false;
            }
        }
            ...
            if (nlapiGetFieldValue('custpage_2663_total_amount') <= 0 && nlapiGetFieldValue('custpage_2663_void_total_amount') <= 0) {
                alert('There are no cheques for processing.');
                result = false;
            }
        }
        else if (paymentType == 'custref') {
        ...
        } else if (paymentType == 'reversal' || paymentType == 'notification') {
    ...
    function canStartPaymentProcessing(fileFormat, account, subsidiaryId) {
        ...
        if (fileFormat && account) {
            ...
            if (res == false) {
                ...
                var errorMessage = 'Another payment process is queued for the selected filters:\n\n';
                errorMessage += 'File Format: ' + fileFormatText + '\n';
                if (accountText) {
    ...
    function checkEntityAmt(){
        ...
        if (isCreditIncluded) {
            ...
            if (invalidEntities.length > 0){
                var message = 'The applied credit amount must not exceed the total payment amount for each entity, per currency. Please check the following entities:\n';
            ...
            if (result && nlapiGetFieldValue('custpage_2663_aggregate') == 'F'){
                if(!confirm("Credit transactions require the Aggregate by Payee option.\nClick OK to automatically check the Aggregate by Payee box and submit the transaction.")){
                    result = false;
                }else{
