...
function snavRefreshPortlets() {
    ...
    var selector = '.portletIconRefresh[onclick]';
    ...
    for (var i = 0; i < els.length; i++) {
        ...
        var tableEl = Ext.get(el).findParent('table');
        if (snavHasNoValue(tableEl)) {
            throw 'Parent table of el not found; el=' + el;
        }
        var titleEl = Ext.get(tableEl).select('.portletTitleText').elements[0];
        if (snavHasNoValue(titleEl)) {
            throw 'Title of table not found; tableEl=' + tableEl;
        }
        ...
        // set an interval for each refresh since not all portlets are refreshed
