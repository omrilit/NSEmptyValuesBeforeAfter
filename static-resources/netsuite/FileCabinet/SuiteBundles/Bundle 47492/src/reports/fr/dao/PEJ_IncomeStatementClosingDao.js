...
TAF.FR.DAO.PEJ_IncomeStatementClosingSAFTLine = function _PEJ_IncomeStatementClosingSAFTLine(id) {
	return {
		id: id,
		internalId: '',
		type: '',
...
TAF.FR.DAO.PEJ_IncomeStatementClosingDao = function _PEJ_IncomeStatementClosingDao() {
...
TAF.FR.DAO.PEJ_IncomeStatementClosingDao.prototype = Object.create(TAF.DAO.SearchDAO.prototype);

TAF.FR.DAO.PEJ_IncomeStatementClosingDao.prototype.createSearchColumns = function _createSearchColumns(params) {
...
TAF.FR.DAO.PEJ_IncomeStatementClosingDao.prototype.createSearchFilters = function _createSearchFilters(params) {
	if(!params) {
    ...
    for (var i=0; i < (params.subIds && params.subIds.length); i++) {
        ...
        var incomesummaryprofitaccount = subsidiarySetting.getFieldValue('incomesummaryprofitaccount');
        var incomesummarylossaccount = subsidiarySetting.getFieldValue('incomesummarylossaccount');
    ...
    this.filters.push(new nlobjSearchFilter('isreversal', null, 'is', 'F'));
    this.filters.push(new nlobjSearchFilter('reversalnumber', null, 'isempty'));
...
TAF.FR.DAO.PEJ_IncomeStatementClosingDao.prototype.rowToObject = function _rowToObject(row) {
	if (!row) {
		throw nlapiCreateError('MISSING_PARAMETER', 'row is required');
    }
    
	var saftLine = new TAF.FR.DAO.PEJ_IncomeStatementClosingSAFTLine(row.getId());
