...
function DE_GODB_RMD_Report(state, params, output, job) {
    this.outline = {
        "Section": ReceivableMasterData
    };
    ...
    function getIndex() {
        ...
        reportIndex.description = 'Kundenübersicht und Transaktionen pro Periode';
    ...
    function ReceivableMasterData() {
        return new DE_GODB_RMD_ReportSection(state, params, output, job);
    }
}

function DE_GODB_RMD_ReportSection(state, params, output, job) {
    TAF.IReportSection.apply(this, arguments);
    this.Name = 'ReceivablesMasterData';
}

DE_GODB_RMD_ReportSection.prototype = Object.create(TAF.IReportSection.prototype);

DE_GODB_RMD_ReportSection.prototype.On_Init = function() {
...
DE_GODB_RMD_ReportSection.prototype.setReportState = function() {
...
DE_GODB_RMD_ReportSection.prototype.On_Header = function() {
    this.output.WriteLine(this.formatter.formatRMDHeader());
};

DE_GODB_RMD_ReportSection.prototype.On_Body = function() {
    try {
        ...
        for (var iperiod = state.periodIndex; iperiod < (params.periodIds && params.periodIds.length); iperiod++) {
            ...
            var index = this.process(this.customerDao, state.searchIndex, this.processRMDLine);
        ...
        nlapiLogExecution('ERROR', 'DE_GODB_RMD_ReportSection.On_Body', ex.toString());
        throw ex;
    }
};

DE_GODB_RMD_ReportSection.prototype.processRMDLine = function(rawLine) {
    if (!rawLine) {
    ...
    try {
        ...
        if (convertedLine) {
            this.output.WriteLine(this.formatter.formatRMDLine(convertedLine));
            this.state[this.Name].txnCustomerIds.push(rawLine.internalId);
        }
    } catch (ex) {
        nlapiLogExecution('ERROR', 'DE_GODB_RMD_ReportSection.processRMDLine', ex.toString());
    }
};

DE_GODB_RMD_ReportSection.prototype.On_CleanUp = function() {
    TAF.IReportSection.prototype.On_CleanUp.call(this);
    delete this.state.common;
};

var DE_GODB_RMD_TXT_Report = function DE_GODB_RMD_TXT_Report(state, params, output, job) {
    params.formatter = new TAF.DE.Formatter.ReceivablesMasterDataFormatter();
    params.filename = 'receivable_master_data.txt';
    DE_GODB_RMD_Report.call(this, state, params, output, job);
};
DE_GODB_RMD_TXT_Report.prototype = Object.create(DE_GODB_RMD_Report.prototype);
DE_GODB_RMD_TXT_Report.IsCRGReport = true;
DE_GODB_RMD_TXT_Report.ReportId = 'DE_GODB_RMD_TXT';
