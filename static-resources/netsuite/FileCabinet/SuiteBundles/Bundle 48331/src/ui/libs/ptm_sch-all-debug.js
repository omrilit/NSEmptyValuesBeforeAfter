...
Ext4.define('Sch.locale.Locale', {
    ...
    bindRequire : function () {
        ...
        // to achieve it we override Ext4.ClassManager.triggerCreated
        var _triggerCreated = Ext4.ClassManager.triggerCreated;

        Ext4.ClassManager.triggerCreated = function(className) {
            _triggerCreated.apply(this, arguments);

            var cls     = Ext4.ClassManager.get(className);
    ...
    apply       : function (classNames) {
        if (this.l10n) {
            ...
            var applyToClass    = function (clsName, cls) {
                cls = cls || Ext4.ClassManager.get(clsName);
                ...
                if (cls && (cls.activeLocaleId !== localeId)) {
                    ...
                    // if it's singleton - apply to it
                        ...
                        cls.l10n = Ext4.apply(cls.l10n || {}, locale);

                    // otherwise we override class
                    } else {
                        Ext4.override(cls, { l10n : locale });
                    }
            ...
            if (classNames) {
                if (!Ext4.isArray(classNames)) classNames = [classNames];

                var name, cls;
                for (i = 0, l = classNames.length; i < l; i++) {
                    if (Ext4.isObject(classNames[i])) {
                        if (classNames[i].singleton) {
                            cls     = classNames[i];
                            name    = Ext4.getClassName(Ext4.getClass(cls));
                        } else {
                            cls     = Ext4.getClass(classNames[i]);
                            name    = Ext4.getClassName(cls);
                        }
                    } else {
                        cls     = null;
                        name    = 'string' === typeof classNames[i] ? classNames[i] : Ext4.getClassName(classNames[i]);
                    }
...
Ext4.define('Sch.util.Patch', {
    ...
    onClassExtended: function(cls, data) {
        ...
        if (data.ieOnly && !Ext4.isIE) {
            return;
        }

        if ((!data.minVersion || Ext4.versions.extjs.equals(data.minVersion) || Ext4.versions.extjs.isGreaterThan(data.minVersion)) &&
            (!data.maxVersion || Ext4.versions.extjs.equals(data.maxVersion) || Ext4.versions.extjs.isLessThan(data.maxVersion))) {
            if (data.applyFn) {
                // Custom override, implementor has full control
                data.applyFn();
            } else {
                // Simple case, just an Ext override
                Ext4.ClassManager.get(data.target).override(data.overrides);
            }
        }
    }
});
...
Ext4.define('Sch.patches.ColumnResize', {
        ...
        if (resizer) {
            ...
            resizer.getConstrainRegion = function() {
                ...
                return me.adjustConstrainRegion(
                    Ext4.util.Region.getRegion(dragHdEl),
                    0,
...
//@PATCH to fix https://www.assembla.com/spaces/bryntum/tickets/869#/activity/ticket
// When resizing columns with fixed locked grid width, columns become smaller due to the default Ext header resizing logic
Ext4.define('Sch.patches.ColumnResizeTree', {
...
Ext4.define('Sch.patches.ElementScroll', {
    override      : 'Sch.mixin.TimelineView',

    _onAfterRender : function() {
        this.callParent(arguments);

        if (Ext4.versions.extjs.isLessThan('4.2.1') || Ext4.versions.extjs.isGreaterThan('4.2.2')) return;
...
*/
Ext4.define('Sch.mixin.Localizable', {
...
Ext4.define("Sch.tooltip.ClockTemplate", {
    ...
    constructor : function() {
        var toRad = Math.PI / 180,
            ...
            hourTop = 3,
            hourLeft = 10,
            isLegacyIE = Ext4.isIE && (Ext4.isIE8m || Ext4.isIEQuirks);
        ...
        function getHourStyleIE(degrees) {
            ...
            return Ext4.String.format("filter:{0};-ms-filter:{0};top:{1}px;left:{2}px;", matrixString, topAdjust+hourTop, leftAdjust+hourLeft);
        }
        ...
        function getMinuteStyleIE(degrees) {
            ...
            return Ext4.String.format("filter:{0};-ms-filter:{0};top:{1}px;left:{2}px;", matrixString, topAdjust+minuteTop, leftAdjust+minuteLeft);
        }

        function getStyle(degrees) {
            return Ext4.String.format("transform:rotate({0}deg);-ms-transform:rotate({0}deg);-moz-transform: rotate({0}deg);-webkit-transform: rotate({0}deg);-o-transform:rotate({0}deg);", degrees);
        }

        this.callParent([
            '<div class="sch-clockwrap {cls}">' +
                '<div class="sch-clock">' +
                    '<div class="sch-hourIndicator" style="{[this.getHourStyle((values.date.getHours()%12) * 30)]}">{[Ext4.Date.monthNames[values.date.getMonth()].substr(0,3)]}</div>' +
...
Ext4.define("Sch.tooltip.Tooltip", {
    ...
    constructor : function(config) {
        ...
        if (!this.template) {
            this.template = Ext4.create("Ext4.XTemplate",
    ...
    show : function(el, xOffset) {
        ...
        this.mon(Ext4.getDoc(), 'mousemove', this.onMyMouseMove, this);
        this.mon(Ext4.getDoc(), 'mouseup', this.onMyMouseUp, this, { single : true });
    },
    ...
    onMyMouseUp : function() {
        this.mun(Ext4.getDoc(), 'mousemove', this.onMyMouseMove, this);
    },
 ...
 */
Ext4.define('Sch.util.Date', {
    requires        : 'Ext4.Date',
    mixins          : ['Sch.mixin.Localizable'],
    singleton       : true,

    // These stem from Ext4.Date in Ext JS but since they don't exist in Sencha Touch we'll need to keep them here
    stripEscapeRe   : /(\\.)/g,
    ...
    setUnitNames : function (unitNames, preserveLocales) {
        ...
        this._unitNames     = Ext4.apply({}, unitNames);
    ...
    compareUnits : function (u1, u2) {
        var ind1 = Ext4.Array.indexOf(this.units, u1),
            ind2 = Ext4.Array.indexOf(this.units, u2);
    ...
    add : function (date, unit, value) {
        ...
        switch (unit.toLowerCase()) {
            ...
            case this.MONTH:
                ...
                if (day > 28) {
                    day = Math.min(day, Ext4.Date.getLastDateOfMonth(this.add(Ext4.Date.getFirstDateOfMonth(date), this.MONTH, value)).getDate());
                }
                d.setDate(day);
    ...
    getUnitByName : function (name) {
        ...
        if (!this.unitsByName[ name ]) {
            Ext4.Error.raise('Unknown unit name: ' + name);
        }
    ...
    getNumberOfMsFromTheStartOfDay : function (date) {
        return date - Ext4.Date.clearTime(date, true) || 86400000;
    },
    ...
    getStartOfNextDay : function (date, clone, noNeedToClearTime) {
        var nextDay = this.add(noNeedToClearTime ? date : Ext4.Date.clearTime(date, clone), this.DAY, 1);

        // DST case
        if (nextDay.getDate() == date.getDate()) {
            var offsetNextDay   = this.add(Ext4.Date.clearTime(date, clone), this.DAY, 2).getTimezoneOffset();
    ...
    getEndOfPreviousDay : function (date, noNeedToClearTime) {
        var dateOnly = noNeedToClearTime ? date : Ext4.Date.clearTime(date, true);
    ...
    runDiagnostics : function() {
        ...
        if (C && C.log) {
            ...
            if (!window.schedulerDebugWin) {
                window.schedulerDebugWin = new Ext4.Window({
                    height:400,
                    width: 500,
        ...
        var els = Ext4.select('.sch-schedulerpanel');
    
        if (els.getCount() === 0) log('No scheduler component found');
    
        var s = Ext4.getCmp(els.elements[0].id),
        ...
        if (eventStore.getCount() > 0) {
            if (!eventStore.first().getStartDate() || !(eventStore.first().getStartDate() instanceof Date)) {
                ...
                log('See Ext JS docs for information about different date formats: http://docs.sencha.com/ext-js/4-0/#!/api/Ext4.Date');
            }
        ...
        if (resourceStore instanceof Ext4.data.TreeStore) resourceStore = resourceStore.nodeStore;
 ...
 **/
Ext4.define('Sch.util.DragTracker', {
    extend      : 'Ext4.dd.DragTracker',
    ...
    getRegion : function() {
        ...
        return new Ext4.util.Region(minY, minX + width, minY + height, minX);
    },
    ...
    onMouseDown: function(e, target){
        ...
        this.startRegion = Ext4.fly(this.dragTarget).getRegion();
        ...
        if (this.autoStart) {
            this.timer =  Ext4.defer(this.triggerStart, this.autoStart === true ? 1000 : this.autoStart, this, [e]);
        }
    },
    ...
    onMouseMove: function(e, target){
        ...
        if (this.active && e.type === 'mousemove' && Ext4.isIE9m && !e.browserEvent.button) {
            e.preventDefault();
            this.onMouseUp(e);
            return;
        }
 ...
 */
Ext4.define('Sch.util.ScrollManager', {
    singleton      : true,
    ...
    constructor : function () {
        this.doScroll = Ext4.Function.bind(this.doScroll, this);
    },
    ...
    startScrollInterval : function (el, dir) {
        ...
        if (Ext4.versions.extjs.isLessThan('4.2.2')) {
            if (dir[0] === 'r') dir = 'left';
...
*/
Ext4.define("Sch.preset.ViewPreset", {
    name                : null,
    ...
    */
    ...
    clone : function () {
        ...
        return new this.self(Ext4.clone(config));
    }    
});
...
Ext4.define('Sch.preset.Manager', {
    extend: 'Ext4.util.MixedCollection',
    requires: [
        'Sch.util.Date',
    ...
    isValidPreset : function(cfg) {
        ...
        for (var o in cfg.headerConfig) {
            if (cfg.headerConfig.hasOwnProperty(o)) {
                valid = valid && Ext4.Array.indexOf(validUnits, cfg.headerConfig[o].unit) >= 0;
            }
        }

        if (cfg.timeResolution) {
            valid = valid && Ext4.Array.indexOf(validUnits, cfg.timeResolution.unit) >= 0;
        }

        if (cfg.shiftUnit) {
            valid = valid && Ext4.Array.indexOf(validUnits, cfg.shiftUnit) >= 0;
        }
            ...
            headerConfig        : {
                ...
                top : {
                    ...
                    renderer    : function(start, end, cfg) {
                        return Sch.util.Date.getShortNameOfUnit("WEEK") + '.' + Ext4.Date.format(start, 'W M Y');
                    }
                }
            }
        },
        ...
        weekAndMonth : {
            ...
            headerConfig        : {
                middle      : {
                    unit    : "WEEK",
                    renderer: function(start, end, cfg) {
                        return Ext4.Date.format(start, 'd M');
                    }
                },
                top         : {
                    unit        : "MONTH",
        ...
        year : {
            ...
            headerConfig        : {
                middle      : {
                    ...
                    align       : 'center',
                    renderer    : function(start, end, cfg) {
                        return Ext4.String.format(Sch.util.Date.getShortNameOfUnit("QUARTER").toUpperCase() + '{0}', Math.floor(start.getMonth() / 3) + 1);
                    }
                },
                top         : {
                    unit        : "YEAR",
            ...
            headerConfig        : {
                bottom  : {
                    ...
                    renderer    : function(start) {
                        return Ext4.Date.dayNames[start.getDay()].substring(0, 1);
                    }
                },
                middle          : {
                    unit        : "WEEK",
...
*/
Ext4.define("Sch.feature.AbstractTimeSpan", {
    ...
    /**
     * @cfg {Ext4.XTemplate} template Template to render the timespan elements  
     */
    template            : null,
    
    /**
     * @cfg {Ext4.data.Store/String} store A store with timespan data, or a string identifying a store.
     */
    store               : null,
    ...
    /**
     * @private
     * @cfg {Ext4.XTemplate} template Template used to render the header elements
     */
    headerTemplate              : null,
    
    
    /**
     * @cfg {String/Ext4.XTemplate} innerHeaderTpl A template providing additional markup to render into each timespan header element
     */
    innerHeaderTpl            : null,
    ...
    //Returns the currently rendered DOM elements of this plugin (if any), as a {@link Ext4.CompositeElementLite} collection.
     ...
     * 
     * @param {Ext4.data.Model} record Data record
    ...
    init:function(scheduler) {
        // TODO COMMENT
        if (Ext4.versions.touch && !scheduler.isReady()) {
            scheduler.on('viewready', function() { this.init(scheduler); }, this);
            return;
        }
        
        if (Ext4.isString(this.innerHeaderTpl)) {
            this.innerHeaderTpl = new Ext4.XTemplate(this.innerHeaderTpl);
        }
        
        var innerHeaderTpl = this.innerHeaderTpl;
        
        if (!this.headerTemplate) {
            this.headerTemplate = new Ext4.XTemplate(
                '<tpl for=".">',
        ...
        this.store = Ext4.StoreManager.lookup(this.store);

        if (!this.store) {
            Ext4.Error.raise("Error: You must define a store for this plugin");
        }
    ...
    onAfterRender : function (scheduler) {
        ...
        if (Ext4.data.NodeStore && view.store instanceof Ext4.data.NodeStore) {
...
//                });
    ...
    refreshSizes : function() {
        ...
        this._resizeTimer = Ext4.Function.defer(this.refreshSizesInternal, this.renderDelay, this);
    },
    ...
    renderElements : function() {
        ...
        this._renderTimer = Ext4.Function.defer(this.renderElementsInternal, this.renderDelay, this);
    },
    ...
    renderBodyElementsInternal : function (records) {
        Ext4.DomHelper.append(this.containerEl, this.generateMarkup(false, records));
    },
    ...
    renderHeaderElementsInternal : function (records) {
        ...
        if (containerEl) {
            Ext4.DomHelper.append(containerEl, this.generateHeaderMarkup(false, records));
        }
    },
    ...
    renderElementsInternal : function() {
        ...
        if (Ext4.versions.extjs && !this.schedulerView.el.down('table')) return;
    ...
    updateBodyElement : function (record) {
        var el = Ext4.get(this.getElementId(record));
    ...
    updateHeaderElement : function (record) {
        var el = Ext4.get(this.getHeaderElementId(record));
    ...
    refreshSingle : function(store, record) {
        Ext4.each(record, this.updateBodyElement, this);
        
        if (this.showHeaderElements) {
            Ext4.each(record, this.updateHeaderElement, this);
        }
    }
}); 
 ...
 */
Ext4.define("Sch.feature.DragCreator", {
    requires : [
        'Ext4.XTemplate',
        'Sch.util.Date',
    ...
    /**
     * @cfg {Ext4.Template} template The HTML template shown when dragging to create new items
     */
    ...
    constructor : function (config) {
        ...
        this.template = this.template || new Ext4.Template(
    ...
    onDrag            : function (tracker, e) {
        ...
        Ext4.apply(dragRegion, me.rowBoundaries);
    ...
    onDragEnd         : function (tracker, e) {
        ...
        Ext4.fly(t).on('click', this.eventSwallower);

        setTimeout(function() {
            Ext4.fly(t).un('click', this.eventSwallower);
        }, 100);
    ...
    finalize : function (doCreate) {
        ...
        if (doCreate) {
            var ev = Ext4.create(schedulerView.eventStore.model);

            // HACK, for the Gantt chart assignments to work properly - we need the task to be in the task store before assigning it
            // to the resource.
            if (Ext4.data.TreeStore && schedulerView.eventStore instanceof Ext4.data.TreeStore) {
                ev.set('leaf', true);
    ...
    setupTooltips : function () {
        ...
        if (me.showHoverTip) {
            ...
            me.hoverTip = new Ext4.ToolTip(Ext4.applyIf({
...
@param {Object} config The object containing the configuration of this model.
*/
Ext4.define("Sch.feature.SchedulerDragZone", {
    ...
    requires    : [
        ...
        // a missing require of Ext4.dd.DragDrop:
        // http://www.sencha.com/forum/showthread.php?276603-4.2.1-Ext4.dd.DragDrop-missing-Ext4.util.Point-in-dependency-quot-requires-quot
        'Ext4.util.Point'
    ],
    ...
    constructor : function (el, config) {
        ...
        if (Ext4.isIE8m && window.top !== window) {
            Ext4.dd.DragDropManager.notifyOccluded = true;
        }
        
        var proxy       = this.proxy = this.proxy || new Ext4.dd.StatusProxy({
            shadow                  : false,
            ...
            ensureAttachedToBody    : Ext4.emptyFn
        });
        ...
        Ext4.dd.ScrollManager.register(schedulerView.el);
    ...
    destroy : function () {
        ...
        Ext4.dd.ScrollManager.unregister(this.schedulerView.el);
    },
    ...
    onDragEnter : Ext4.emptyFn,
    onDragOut   : Ext4.emptyFn,
    
    
    setVisibilityForSourceEvents : function (show) {
        Ext4.each(this.dragData.eventEls, function (el) {
            el[ show ? 'show' : 'hide' ]();
        });
    },
    ...
    getDragData: function (e) {
        ...
        Ext4.Array.each(relatedRecords, function (r) {
        ...
        var dragData = {
            ...
            bodyScroll          : Ext4.getBody().getScroll(),
    ...
    startDrag : function() {
        if (this.enableCopy) {
            Ext4.EventManager.on(document, "keydown", this.onKey, this);
            Ext4.EventManager.on(document, "keyup", this.onKey, this);
        }
    ...
    endDrag : function() {
        if (this.enableCopy) {
            Ext4.EventManager.un(document, "keydown", this.onKey, this);
            Ext4.EventManager.un(document, "keyup", this.onKey, this);
        }
        this.callParent(arguments);
    },
    ...
    updateRecords : function (context) {
        ...
        var timeDiff            = context.timeDiff,
            isTreeStore         = Ext4.data.TreeStore && resourceStore instanceof Ext4.data.TreeStore;
        ...
        Ext4.each(context.relatedEventRecords, function (related) {
            ...
            related.shift(Ext4.Date.MILLI, timeDiff);
    ...
    resolveResource : function (xy, e) {
        ...
        if (Ext4.isIE8 && e && e.browserEvent.synthetic) {
        ...
        if (!node.className.match(view.timeCellCls)) {
            var parent = Ext4.fly(node).up('.' + view.timeCellCls);
     ...
     * @param {Ext4.Element} sourceEl The event element that is the source drag element
    ...
    getDragElement : function(sourceEl, dragData) {
        ...
        if (eventEls.length > 1) {
            var ctEl            = Ext4.core.DomHelper.createDom({
                tag     : 'div',
                cls     : 'sch-dd-wrap',
                style   : { overflow : 'visible' }
            });

            Ext4.Array.each(eventEls, function (el) {
                copy            = el.dom.cloneNode(true);
                
                copy.id         = el.dom === sourceEl.dom ? "sch-id-dd-ref" : Ext4.id();
    ...
    onDragDrop: function (e, id) {
        ...
        var me          = this,
            s           = me.schedulerView,
            target      = me.cachedTarget || Ext4.dd.DragDropMgr.getDDById(id),
        ...
        if (updateRecords){
            ...
            if (!updated) {
                ...
                if (Ext4.isIE9) {
                    ...
                    Ext4.Function.defer(me.onValidDrop, 10, me, dragData.ddCallbackArgs);
                } else {
 ...
 * @param {Object} config The object containing the configuration of this model.
 */
Ext4.define("Sch.feature.DragDrop", {
    ...
    initProxyLessDD : function() {
        ...
        s.eventDragZone = new Sch.feature.SchedulerDragZone(s.ownerCt.el, Ext4.apply({
            ddGroup             : s.id,
            schedulerView       : s,
...
Ext4.define("Sch.feature.ResizeZone" , {
    extend      : "Ext4.util.Observable",
    requires    : [
        'Ext4.resizer.Resizer',
        'Sch.tooltip.Tooltip'
    ],
    ...
    onMouseDown : function(e, t) {
        ...
        this.origEl         = Ext4.get(e.getTarget('.sch-event'));
    ...
    onMouseMove : function(e, t) {
        ...
        if (this.showTooltip) {
             if (!this.tip) {
                this.tip = Ext4.create("Sch.tooltip.Tooltip", {
                    schedulerView   : s,
                    cls             : 'sch-resize-tip'
                });
            }
    ...
    createResizer : function (el, eventRecord, handlePos) {
        ...
        var resizer = new Ext4.resizer.Resizer(resizerCfg);
        ...
        if (resizer.resizeTracker) {
            ...
            resizer.resizeTracker.updateDimensions = function(e) {
                if (!Ext4.isWebKit || e.getTarget('.sch-timelineview')) {
                    var scrollDelta;
            ...
            resizer.resizeTracker.resize = function(box) {
                ...
                Ext4.resizer.ResizeTracker.prototype.resize.apply(this, arguments);
            };
        }
...
Ext4.define("Sch.eventlayout.Horizontal", {
    ...
    layoutEventsInBands: function (bandIndex, events) {
        ...
        do {
            ...
            while (event) {
                ...
                Ext4.Array.remove(events, event);
...
*/
Ext4.define("Sch.eventlayout.Vertical", {
    ...
    getPriorOverlappingEvents : function(events, event) {
        ...
        for (var i = 0, l = Ext4.Array.indexOf(events, event); i < l ; i++) {
...
*/
Ext4.define("Sch.column.Resource", {
    extend          : "Ext4.grid.Column",
...
*/
Ext4.define("Sch.view.model.TimeAxis", {
    extend: 'Ext4.util.Observable',
    ...
    */
    update: function (availableWidth, suppressEvent) {
        ...
        if (!Ext4.isNumber(this.availableWidth)) {
        ...
        if (!Ext4.isNumber(tickWidth) || tickWidth <= 0) {
    ...
    createHeaderRow: function (position, headerConfig) {
        var cells   = [],
            ...
            today   = Ext4.Date.clearTime(new Date());
        ...
        me.forEachInterval(position, function (start, end, i) {
            ...
            if (headerConfig.renderer) {
                ...
                colConfig.header = Ext4.Date.format(start, headerConfig.dateFormat);
            }
            ...
            if (headerConfig.unit === Sch.util.Date.DAY && (!headerConfig.increment || headerConfig.increment === 1)) {
                ...
                if (Ext4.Date.clearTime(start, true) - today === 0) {
    ...
    */
    ...
    getPositionFromDate: function (date) {
        ...
        if (tick >= 0) {
            pos     = this.tickWidth * (tick - this.timeAxis.visibleTickStart);
        }
    ...
    */
    getTotalWidth: function () {
        return this.tickWidth * this.timeAxis.getVisibleTickTimeSpan();
    },
...
//        }
    ...
    */
    ...
    setViewRowHeight : function (value, suppressEvent) {
        ...
        var property        = 'rowHeight' + Ext4.String.capitalize(this.orientation);
...
* Normally you should not interact with this class directly.
*/
Ext4.define("Sch.view.HorizontalTimeAxis", {
    extend: 'Ext4.util.Observable',
    ...
    */
    ...
    constructor: function (config) {
        var me = this;
        var isTouch = !!Ext4.versions.touch;
        ...
        me.containerEl = Ext4.get(me.containerEl);

        if (!(me.headerHtmlRowTpl instanceof Ext4.Template)) {
            me.headerHtmlRowTpl = me.headerHtmlRowTpl.replace('{{baseCls}}', this.baseCls).replace('{{tableCls}}', this.tableCls);
            me.headerHtmlRowTpl = new Ext4.XTemplate(me.headerHtmlRowTpl);
        }
    ...
    getHTML : function (header, meta, column) {
        ...
        var nbrRows             = Ext4.Object.getKeys(columnConfig).length;
    ...
    embedCellWidths : function (cells) {
        // For desktop only, flags such as Ext4.isSafari only exist in Ext JS (in touch it's set in Ext4.os)
        var widthAdjust     = (Ext4.isIE7 || Ext4.isSafari) ? 1 : 0;
    ...
    onElClick: function(eventName) {
        return function (event, target) { 
            ...
            var position        = Ext4.fly(target).getAttribute('headerPosition'),
                index           = Ext4.fly(target).getAttribute('headerIndex'),
    ...
    highlightCell: function (e, cell) {
        ...
        if (cell !== me.highlightedCell) {
            ...
            Ext4.fly(cell).addCls(me.hoverCls);
        }
    },
    ...
    clearHighlight: function () {
        ...
        if (highlighted) {
            Ext4.fly(highlighted).removeCls(me.hoverCls);
 ...
 */
Ext4.define("Sch.column.timeAxis.Horizontal", {
    ...
    getAvailableWidthForSchedule : function() {
        ...
        return available - Ext4.getScrollbarSize().width - 1;
    },
 ...
 * @param {Object} config The configuration options
 */
Ext4.define('Sch.column.timeAxis.Vertical', {
    ...
    renderer : function (val, meta, record, rowIndex) {
        ...
        if (hc.renderer) {
            ...
            return Ext4.Date.format(record.data.start, hc.dateFormat);
        }
    }
});
 ...
 * @extends Ext4.grid.locking.Lockable.
 * @private
 * This is a private class for internal use.
 */
Ext4.define('Sch.mixin.Lockable', {
    extend                      : 'Ext4.grid.locking.Lockable',
    ...
    injectLockable: function () {
        ...
        var me          = this;
        var isTree      = Ext4.data.TreeStore && me.store instanceof Ext4.data.TreeStore;
        ...
        me.lockedGridConfig = Ext4.apply({}, me.lockedGridConfig || {});
        me.normalGridConfig = Ext4.apply({}, me.schedulerConfig || me.normalGridConfig || {});
        ...
        Ext4.applyIf(me.lockedGridConfig, {
            useArrows           : true,
        ...
        Ext4.applyIf(me.normalGridConfig, {
        ...
        if (lockedGrid.width) {
            ...
            me.syncLockedWidth = Ext4.emptyFn;
        ...
        if (isTree) {
            ...
            var oldOnUpdate = Ext4.tree.View.prototype.onUpdate;
            lockedViewConfig.onUpdate = function() {
                ...
                Ext4.resumeLayouts();
                this.refreshSize = Ext4.tree.View.prototype.refreshSize;
            };
        ...
        // Now post processing, changing and overriding some things that Ext4.grid.Lockable sets up
        ...
        if (Ext4.getScrollbarSize().width === 0) {
        ...
        if (lockedView.store !== normalView.store) {
            Ext4.Error.raise('Sch.mixin.Lockable setup failed, not sharing store between the two views');
        }
    ...
    patchSyncHorizontalScroll : function(grid) {
        // Override scrollTask
        grid.scrollTask = new Ext4.util.DelayedTask(function (left, setBody) {
    ...
    updateSpacer : function() {
        ...
        if (lockedView.rendered && normalView.rendered && lockedView.el.child('table')) {
            ...
            me.spacerEl = Ext4.getDom(spacerId);

            // HACK ie 6-7 and 8 in quirks mode fail to set style of hidden elements, so we must remove it manually
            if (Ext4.isIE6 || Ext4.isIE7 || (Ext4.isIEQuirks && Ext4.isIE8) && me.spacerEl) {

                Ext4.removeNode(me.spacerEl);
                me.spacerEl = null;
            }
            ...
            if (me.spacerEl) {
                ...
                Ext4.core.DomHelper.append(spacerParent, {
                    id      : spacerId,
//                    cls     : this.store.buffered ? 'sch-locked-buffered-spacer' : '',
    ...
    patchSubGrid : function (grid, isLocked) {
        ...
        // Ext4.grid.plugin.BufferedRendererTreeView override.
        ...
        // but, this method is not called for normal grid, because its a regular grid view, w/o Ext4.grid.plugin.BufferedRendererTreeView
        // so we call that method manually
        var isTree                  = Ext4.data.TreeStore && this.store instanceof Ext4.data.TreeStore;
...
//        }
        ...
        // The buffered renderer plugin includes 2 overrides for grid view: Ext4.grid.plugin.BufferedRendererTableView and 
        // Ext4.grid.plugin.BufferedRendererTreeView.
        // Seems the buffered renderer behavior is completely broken when doing CRUD with the store (nodes are inserted in
        // random places in the view), that is why Ext4.grid.plugin.BufferedRendererTreeView modestly does a full refresh 
    ...
    patchBufferedRenderingPlugin : function (plugin) {
        ...
        if (Ext4.getVersion('extjs').isLessThan('4.2.1.883')) {
    ...
    patchViews : function() {
        if (Ext4.isIE) {
            ...
            var eventName   = Ext4.getVersion('extjs').isLessThan('4.2.2.1144') ? 'mousedown' : 'click';
            ...
            selModel.processSelection = function (view, record, item, index, e) {
                ...
                if (e.type == eventName) {
                    ...
                    lockedView.scrollRowIntoView    = normalView.scrollRowIntoView = Ext4.emptyFn;
                    lockedView[ focusMethod ]       = normalView[ focusMethod ] = Ext4.emptyFn;
                }
...
//            lockedView.focusRow = function (row, delay) { return oldLockedFocusRow.call(this, row, 0) };
            ...
            if (Ext4.tree && Ext4.tree.plugin && Ext4.tree.plugin.TreeViewDragDrop) {

                lockedView.on('afterrender', function() {
                    Ext4.each(lockedView.plugins, function(plug) {

                        if (plug instanceof Ext4.tree.plugin.TreeViewDragDrop) {
                            ...
                            plug.dragZone.view.on('itemmousedown', function() {
                                ...
                                lockedView[ focusMethod ] = Ext4.emptyFn;
...
if (!Ext4.ClassManager.get("Sch.model.Customizable")) {

    Ext4.define('Sch.model.Customizable', {
        ...
        onClassExtended : function (cls, data, hooks) {
            ...
            hooks.onBeforeCreated = function (cls, data) {
                ...
                Ext4.Array.each(customizableFields, function (field) {
                    // normalize to object
                ...
                // already processed by the Ext4.data.Model `onBeforeCreated`
                ...
                Ext4.Object.each(customizableFieldsByName, function (name, customizableField) {
                    ...
                    if (fields.containsKey(realFieldName)) {
                        ...
                        customizableFields.push(
                            new Ext4.data.Field(
                                Ext4.applyIf({ name : stableFieldName, isCustomizableField : true }, fields.getByKey(realFieldName))
                            )
                        );
                    } else
                        // we create a new copy of the `customizableField` using possibly new name
                        fields.add(new Ext4.data.Field(Ext4.applyIf({ name : realFieldName, isCustomizableField : true }, customizableField)));

                    var capitalizedStableName  = Ext4.String.capitalize(stableFieldName);
...
/**
 ...
 Its a subclass of the {@link Sch.model.Customizable}, which is in turn subclass of {@link Ext4.data.Model}.
...
Ext4.define('Sch.model.Range', {
    ...
    getDates : function () {
        ...
        for (var date = Ext4.Date.clearTime(this.getStartDate(), true); date < endDate; date = Sch.util.Date.add(date, Sch.util.Date.DAY, 1)) {
    ...
    forEachDate : function (func, scope) {
        return Ext4.each(this.getDates(), func, scope);
    },
 ...
 */
Ext4.define('Sch.model.TimeAxisTick', {
...
This class represent a single event in your schedule. Its a subclass of the {@link Sch.model.Range}, which is in turn subclass of {@link Sch.model.Customizable} and {@link Ext4.data.Model}.
...
--------------------
    ...
    Ext4.define('MyProject.model.Event', {
...
Ext4.define('Sch.model.Event', {
    ...
    */
    getResource: function (resourceId) {
        if (this.stores && this.stores.length > 0) {
            ...
            if (Ext4.data.TreeStore && rs instanceof Ext4.data.TreeStore) {
    ...
    */
    setResource: function (resourceOrId) {
        this.set(this.resourceIdField, (resourceOrId instanceof Ext4.data.Model) ? resourceOrId.getId() || resourceOrId.internalId : resourceOrId);
    },
    ...
    */
    isPersistable: function () {
        ...
        Ext4.each(resources, function(r) {
            if (r.phantom) {
                persistable = false;
                return false;
            }
...
/**
 ...
 This class represent a single Resource in the scheduler chart. Its a subclass of the {@link Sch.model.Customizable}, which is in turn subclass of {@link Ext4.data.Model}.
 ...
 Ext4.define('MyProject.model.Resource', {
...
// Don't redefine the class, which will screw up instanceof checks etc
if (!Ext4.ClassManager.get("Sch.model.Resource")) {

    Ext4.define('Sch.model.Resource', {
        extend : 'Sch.model.Customizable',
...
*/
Ext4.define("Sch.data.mixin.EventStore", {
    ...
    onResourceStoreWrite: function (store, operation) {
        if (operation.wasSuccessful()) {
            ...
            Ext4.each(rs, function(resource) {
    ...
    * @return {Ext4.util.MixedCollection} the events
    */
...
*/
Ext4.define("Sch.data.EventStore", {
    ...
    constructor : function() {
        ...
        var newC = function () {};
        newC.prototype = Ext4.ModelManager.getModel(this.getModel()).prototype;
 ...
 * 
 */
Ext4.define("Sch.data.mixin.ResourceStore", {
...
Ext4.define("Sch.data.FilterableNodeStore", {
    extend          : 'Ext4.data.NodeStore',
...
This is a mixin for the Ext4.data.TreeStore providing filtering functionality. Please note, that Ext JS does not support filtering of tree stores,
and the functionality of this mixin is not related to the standard Ext JS store filtering (which utilizes Ext4.util.Filter etc). This implementation should however be flexible
...
*/
Ext4.define("Sch.data.mixin.FilterableTreeStore", {
    ...
    createNodeStore : function (treeStore) {
        return Ext4.create(this.nodeStoreClassName, {
            treeStore       : treeStore,
            recursive       : true,
    ...
    filterTreeBy : function (params, scope) {
        ...
        if (arguments.length == 1 && Ext4.isObject(arguments[ 0 ])) {
            scope       = params.scope;
...
Its a subclass of "Ext4.data.Store" - a store with linear data presentation.

*/
Ext4.define("Sch.data.ResourceStore", {
    extend  : 'Ext4.data.Store',
 ...
 * 
 * This is a class holding all the resources to be rendered into a {@link Sch.panel.SchedulerTree}. It is a subclass of "Ext4.data.TreeStore" - a store containing hierarchical data.
 * 
 ...
 */
Ext4.define("Sch.data.ResourceTreeStore", {
...
Ext4.define("Sch.data.TimeAxis", {
    ...
    */    
    ...
    // these dates will contain adjusted start/end (with the full ticks)
    adjustedStart       : null,
    adjustedEnd         : null,
    // the visible position in the first tick, is always 0 <= value < 1, in case of autoAdjust - always = 0
    ...
    constructor : function(config) {
        ...
        me.on(Ext4.versions.touch ? 'refresh' : 'datachanged', function(ta, newPreset, suppressRefresh) {
    ...
    */
    reconfigure : function (config, suppressRefresh) {
        ...
        var adjusted        = this.getAdjustedDates(config.start, config.end);
        
        var start           = adjusted.start;
        var end             = adjusted.end;
        
        if (this.fireEvent('beforereconfigure', this, start, end) !== false) {
            var ticks               = this.generateTicks(start, end, this.unit, this.increment || 1, this.mainUnit);
            
            // Distinguish between altering the properties of the timeAxis as opposed to
            // traversing it, changing the start/end dates
            var nbrConfigKeys       = Ext4.Object.getKeys(config).length;
            ...
            if (this.getCount() === 0) {
                Ext4.Error.raise('Invalid time axis configuration or filter, please check your input data.');
            }
            this.resumeEvents();
            
            this.adjustedStart      = adjusted.start;
            // in case of auto-adjusting, the end date is not just adjusted to the unit, it is also adjusted to the 
            // increment (inside of `generateTicks` method), so the only way to get the correct value is `getEnd`
            this.adjustedEnd        = this.autoAdjust ? this.getEnd() : adjusted.end;
            
            this.visibleTickStart   = (this.getStart() - this.adjustedStart) / Sch.util.Date.getUnitDurationInMs(this.mainUnit);
            this.visibleTickEnd     = this.getCount() - (this.adjustedEnd - this.getEnd()) / Sch.util.Date.getUnitDurationInMs(this.mainUnit);
    ...
    */
    ...
    filterBy : function(fn, scope) {
        ...
        if (this.getCount() === 0) {
            this.clearFilter();
            this.resumeEvents();
            Ext4.Error.raise('Invalid time axis filter - no ticks passed through the filter. Please check your filter method.');
        }
        this.resumeEvents();
    },
    ...
    getVisibleTickTimeSpan : function () {
        return this.visibleTickEnd - this.visibleTickStart;
    },
    ...
    getTickFromDate : function (date) {
        ...
        if (this.isContinuous()) {
            ...
            for (var i = 0; i <= lastTickIndex; i++) {
                tickEnd         = i == lastTickIndex ? adjustedEnd : ticks[ i ].data.end;
    
                if (date <= tickEnd) {
                    tickStart   = i === 0 ? adjustedStart : ticks[ i ].data.start;
                        
                    tick        = i + (date - tickStart) / (tickEnd - tickStart);
    ...
    */
    getDateFromTick : function (tick, roundingMethod) {
        ...
        var end         = wholeTick == this.getCount() - 1 ? this.adjustedEnd : tickData.end;
    ...
    */
    ...
    roundDate : function(date) {
        ...
        switch(this.resolutionUnit) {
            ...
            case Sch.util.Date.MONTH:
                var nbrMonths = Sch.util.Date.getDurationInMonths(relativeTo, dt) + (dt.getDate() / Ext4.Date.getDaysInMonth(dt)),
 ...
 */
Ext4.define("Sch.view.Horizontal", {
    requires : [
        'Ext4.util.Region',
        'Ext4.Element',
        'Sch.util.Date'
    ...
    getEventRenderData: function (event, start, end) {
        ...
        if (view.managedEventSizing) {
            data.top = M.max(0, (view.barMargin - ((Ext4.isIE && !Ext4.isStrict) ? 0 : view.eventBorderWidth - view.cellTopBorderWidth)));
    ...
    /**
    * Gets the Ext4.util.Region, relative to the page, represented by the schedule and optionally only for a single resource. This method will call getDateConstraints to 
    ...
    * @return {Ext4.util.Region} The region of the schedule
    */
    ...
    /**
    * Gets the Ext4.util.Region, relative to the scheduling view element, representing the passed resource and optionally just for a certain date interval.
    * @param {Sch.model.Resource} resourceRecord The resource record
    * @param {Date} startDate A start date constraining the region
    * @param {Date} endDate An end date constraining the region
    * @return {Ext4.util.Region} The region of the resource
    */
    getResourceRegion: function (resourceRecord, startDate, endDate) {
        var view        = this.view,
            rowNode     = view.getRowNode(resourceRecord),
            offsets     = Ext4.fly(rowNode).getOffsetsTo(view.getEl()),
            taStart     = view.timeAxis.getStart(),
            taEnd       = view.timeAxis.getEnd(),
            ...
            bottom      = offsets[1] + Ext4.fly(rowNode).getHeight() - view.cellBottomBorderWidth;

        if (!Ext4.versions.touch) {
    ...
    *  @return {Ext4.util.Region} The region for the time span
    */
    getTimeSpanRegion: function (startDate, endDate, useViewSize) {
        ...
        return new Ext4.util.Region(0, Math.max(startX, endX), height, Math.min(startX, endX));
    },
    ...
    */
    ...
    onEventAdd: function (s, events) {
        ...
        Ext4.Object.each(affectedResources, function (id, resource) {
    ...
    onEventRemove: function (s, eventRecords) {
        ...
        var isTree          = Ext4.tree && Ext4.tree.View && view instanceof Ext4.tree.View;

        if (!Ext4.isArray(eventRecords)) {
        ...
        for (var i = 0; i < eventRecords.length; i++) {
            ...
            if (resources.length > 1) {
                Ext4.each(resources, updateResource, this);
            } else {
                ...
                if (node) {
                    ...
                    // Note, the methods below should not rely on Ext4.get but since Ext4.anim.run
                    ...
                    if (Ext4.Element.prototype.fadeOut) {
                        Ext4.get(node).fadeOut({
                            callback: function() { updateResource(resource); }
                        });
                    } else {
                        Ext4.Anim.run(Ext4.get(node), 'fade', {
                            out         : true,
                            duration    : 500,
    ...
    onEventUpdate: function (store, model, operation) {
        ...
        Ext4.each(resources, function(resource) {
            view.repaintEventsForResource(resource, true);
        });
    },
...
Ext4.define("Sch.view.Vertical", {
    ...
    getScheduleRegion: function (resourceRecord, eventRecord) {
        var view        = this.view,
            region      = resourceRecord ? Ext4.fly(view.getScheduleCell(0, view.resourceStore.indexOf(resourceRecord))).getRegion() : view.getTableRegion(),
        ...
        return new Ext4.util.Region(Math.min(startY, endY), right, Math.max(startY, endY), left);
    },
    ...
    /**
    * Gets the Ext4.util.Region representing the passed resource and optionally just for a certain date interval.
    * @param {Sch.model.Resource} resourceRecord The resource record
    * @param {Date} startDate A start date constraining the region
    * @param {Date} endDate An end date constraining the region
    * @return {Ext4.util.Region} The region of the resource
    */
    ...
    resolveResource: function (el) {
        var view = this.view;
        el = Ext4.fly(el).is(view.timeCellSelector) ? el : Ext4.fly(el).up(view.timeCellSelector);

        if (el) {
            var node = el.dom ? el.dom : el;
            var index = Ext4.Array.indexOf(Array.prototype.slice.call(node.parentNode.children), node);
    ...
    renderSingle : function (event) {
        ...
        if (existing) {
            Ext4.fly(existing).destroy();
        }

        var containerCell   = Ext4.fly(view.getScheduleCell(0, rIndex));
    ...
    */
    getTimeSpanRegion: function (startDate, endDate) {
        ...
        return new Ext4.util.Region(Math.min(startY, endY), width, Math.max(startY, endY), 0);
    },
...
@extends Ext4.selection.Model

This class provides the basic implementation event selection in a grid.

*/
Ext4.define("Sch.selection.EventModel", {
    extend      : 'Ext4.selection.Model',

    alias       : 'selection.eventmodel',

    requires    : [ 'Ext4.util.KeyNav' ],
    ...
    onVetoUIEvent : Ext4.emptyFn, // Some bug in Ext JS 4.2.1 - should be removed later
...
Plugin (ptype = 'scheduler_printable') for printing an Ext Scheduler instance. Please note that this will not generate a picture perfect
 ...
 To use this plugin, add it to scheduler as usual. The plugin will add an additional `print` method to the scheduler:
        ...
        var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
            ...
...
*/
Ext4.define("Sch.plugin.Printable", {
    extend          : 'Ext4.AbstractPlugin',
    ...
    init : function(scheduler) {
        ...
        scheduler.print = Ext4.Function.bind(this.print, this);
    },

    // private, the template for the new window
    mainTpl : new Ext4.XTemplate('{docType}' +
          '<html class="' + Ext4.baseCSSPrefix + 'border-box {htmlClasses}">' +
            '<head>' +
          ...
          '</html>',
          {
            ...
            printLockedGrid: function(values){
                ...
                if (values.lockedGrid){
                    str += '<div id="lockedRowsCt" style="left:-'+values.lockedScroll+'px;margin-right:-'+values.lockedScroll+'px;width:'+(values.lockedWidth + values.lockedScroll)+'px;top:'+values.headerHeight+'px;" class="sch-print-locked-rows-ct ' + values.innerLockedClasses + ' ' + Ext4.baseCSSPrefix + 'grid-inner-locked">';
    ...
    getGridContent : function(component) {
        ...
        if (Ext4.versions.extjs.isLessThan('4.2.1')) {
            lockedGrid.headerCt.items.each(function(column, i) {
                if (column.isHidden()) {
                    Ext4.fly(div).down('colgroup:nth-child(' + (i+1) + ') col').setWidth(0);
                }
            });
        }
        ...
        if (Sch.feature && Sch.feature.AbstractTimeSpan) {
            ...
            Ext4.each(toIterate, function(plug) {
        ...
        return {
            ...
            lockedWidth        : lockedWidth - (Ext4.isWebKit ? 1 : 0),
    ...
    getStylesheets : function() {
        return Ext4.getDoc().select('link[rel="stylesheet"]');
    },
    ...
    print : function() {
        ...
        if (!(this.mainTpl instanceof Ext4.Template)) {
            // Compile the tpl upon first call
            var headerRowHeight = 22;

            this.mainTpl = new Ext4.XTemplate(this.mainTpl, {
                compiled : true,
                disableFormats : true
            });
        }
        ...
        var v = component.getView(),
            ...
            ctTmp = Ext4.get(Ext4.core.DomHelper.createDom({
                tag : 'div'
            })),
            styleFragment;
        ...
        var gridContent = this.getGridContent(component),
            html = this.mainTpl.apply(Ext4.apply({
                waitText            : this.waitText,
                docType             : this.docType,
                htmlClasses         : Ext4.getBody().parent().dom.className,
                bodyClasses         : Ext4.getBody().dom.className,
                ...
                setupScript         : ("window.onload = function(){ (" + this.setupScript.toString() + ")(" +
                    component.syncRowHeight + ", " + this.autoPrintAndClose + ", " + Ext4.isChrome + ", " + Ext4.isIE +
                "); };")
            }, gridContent));
...
Ext4.define('Sch.plugin.Export', {
    ...
    */
    ...
    constructor : function (config) {
        ...
        if (config.exportDialogConfig) {
            Ext4.Object.each(this.defaultConfig, function(k, v, o){
        ...
        if (!this.tpl) {
            this.tpl = new Ext4.XTemplate('<!DOCTYPE html>' +
                '<html class="' + Ext4.baseCSSPrefix + 'border-box {htmlClasses}">' +
                    '<head>' +
                        '<meta content="text/html; charset=UTF-8" http-equiv="Content-Type" />' +
                        ...
                        '{styles}' +
                    '</head>' +
                    '<body class="' + Ext4.baseCSSPrefix + 'webkit sch-export {bodyClasses}">' +
                        '<tpl if="showHeader">' +
    ...
    init : function (scheduler) {
        ...
        scheduler.showExportDialog = Ext4.Function.bind(this.showExportDialog, this);
        scheduler.doExport         = Ext4.Function.bind(this.doExport, this);
    },
    ...
    */
    showExportDialog : function() {
        ...
        me.win  = Ext4.create(me.exportDialogClassName, {
            plugin                  : me,
            exportDialogConfig      : Ext4.apply({
    ...
    */
    getStylesheets : function() {
        var styleSheets = Ext4.getDoc().select('link[rel="stylesheet"]'),
            ctTmp = Ext4.get(Ext4.core.DomHelper.createDom({
                tag : 'div'
            })),
            stylesString;
    ...
    */
    doExport : function (conf, callback, errback) {
        ...
        if (config.range !== 'complete') {
            ...
            ticks       = Ext4.Array.filter(ticks, function (tick, index) {
                if (index < startTick) {
                    skippedColsBefore++;
                    return false;
        ...
        // single page mode
            ...
            var sizeInInches    = me.getRealSize(),
                width           = Ext4.Number.toFixed(sizeInInches.width / me.DPI, 1),
                height          = Ext4.Number.toFixed(sizeInInches.height / me.DPI, 1);
        ...
        if (me.printServer) {
            ...
            // for debugging mode we just show output instead of sending it to server
                ...
                var w, a = Ext4.JSON.decode(htmlArray);
        ...
        if (me.test) {
            return {
                htmlArray       : Ext4.JSON.decode(htmlArray),
    ...
    */
    getRealSize : function() {
        var component    = this.scheduler,
            ...
            height       = (headerHeight + component.lockedGrid.getView().getEl().down('.' + Ext4.baseCSSPrefix + 'grid-table').getHeight()),
            width        =  (component.lockedGrid.headerCt.getEl().first().getWidth() +
                component.normalGrid.body.select('.' + Ext4.baseCSSPrefix + 'grid-table').first().getWidth());
    ...
    */
    getExportJsonHtml : function (calculatedPages, params) {
        var me                  = this,
            ...
            re                  = new RegExp(Ext4.baseCSSPrefix + 'ie\\d?|' + Ext4.baseCSSPrefix + 'gecko', 'g'),
            bodyClasses         = Ext4.getBody().dom.className.replace(re, ''),
            componentClasses    = component.el.dom.className,
            ...
            for (var i = 0; i < columnPages; i++) {
                ...
                if (lockedColumnPages && i < lockedColumnPagesLen) {
                    if (i === lockedColumnPagesLen - 1 && columnsAmountLocked !== 0) {
                        ...
                        lastColumn = Ext4.Number.constrain((columnsAmountLocked-1), 0, (ticks.length - 1));
        ...
        return Ext4.JSON.encode(htmlArray);
    },
    ...
    *
    * @return {Object} frag Ext4.dom.Element with resized html.
    */
    resizePanelHTML: function (HTML) {
        //create empty div that will temporarily hold our panel current HTML
        var frag       = Ext4.get(Ext4.core.DomHelper.createDom({
                tag: 'div',
                html: HTML.dom
            })),
        ...
        if (Ext4.isIE6 || Ext4.isIE7 || Ext4.isIEQuirks){
            ...
            Ext4.Array.each(lockedElements, function(el){
                if(el !== null){
                    el.style.height = '100%';
                    el.style.width  = '100%';
                }
            ...
            Ext4.Array.each(normalElements, function(el, idx){
                if (el !== null){
                    if (idx === 1){
                        el.style.width = '100%';
                    } else {
            ...
            Ext4.Array.each(lockedElements, function(el, idx){
                if(el){
                    el.setHeight('100%');
                    if (idx !== 3 && idx !== 2 ) {
                        el.setWidth('100%');
...
To add this plugin to scheduler:
        ...
        var dayStore    = new Ext4.data.Store({
        ...
        var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
            ...
            plugins         : [
                Ext4.create('Sch.plugin.Lines', { store : dayStore })
            ]
        });


*/
Ext4.define("Sch.plugin.Lines", {
    ...
    /**
     * @cfg {String/Ext4.XTemplate} innerTpl A template providing additional markup to render into each timespan element
     */
    innerTpl            : null,
    ...
    getElementData : function(viewStart, viewEnd, records) {
        ...
        for (var i = 0, l = rs.length; i < l; i++) {
            ...
            if (date && Sch.util.Date.betweenLesser(date, viewStart, viewEnd)) {
                ...
                templateData = Ext4.apply({}, this.getTemplateData(record));
...
To add this plugin to scheduler:
        ...
        var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
            ...
            plugins         : [
                Ext4.create('Sch.plugin.CurrentTimeLine', { updateInterval : 30000 })
            ]
        });


*/
Ext4.define("Sch.plugin.CurrentTimeLine", {
...
To add this plugin to scheduler:
    ...
    var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
        ...
        plugins         : [
            Ext4.create('Sch.plugin.DragSelector')
        ]
    });


*/
Ext4.define("Sch.plugin.DragSelector", {
    ...
    onSchedulerDestroy : function () {
        if (this.proxy) Ext4.destroy(this.proxy);
...
@extends Ext4.form.Panel

A plugin (ptype = 'scheduler_eventeditor') used to edit event start/end dates as well as any meta data. It inherits from {@link Ext4.form.FormPanel} so you can define any fields and use any layout you want.
...
Note, that you can customize the start date, start time and duration fields with appropriate configuration options: {@link #dateConfig}, {@link #timeConfig}, {@link #durationConfig}
        ...
        var eventEditor    = Ext4.create('Sch.plugin.EventEditor', {
            ...
            timeConfig      : {
                minValue    : '08:00',
                maxValue    : '18:00'
...
you need to decorate them with a special CSS class so the editor will stay open when clicking 'outside' of the editor element.
    ...
    var myComboBox = new Ext4.form.ComboBox({
        width           : 70,
        ...
    });
 ...
 */
Ext4.define("Sch.plugin.EventEditor", {
     ...
     * @property {Ext4.form.field.Date} startDateField
     */
    startDateField  : null,

    /**
     * This property provides access to the start time field
     * @property {Ext4.form.field.Time} startTimeField
     */
    startTimeField  : null,

    /**
     * This property provides access to the duration spinner field
     * @property {Ext4.form.field.Number} durationField
     */
    durationField   : null,
    ...
    /**
     * @cfg {Object} durationConfig A custom config object that is used to configure the {@link Ext4.form.field.Number duration field}.
     */
    durationConfig  : null,
     ...
     * Example:
        ...
        fieldsPanelConfig : {
            ...
            items       : [
                titleField      = new Ext4.form.TextField({
                    name            : 'Title',
                    fieldLabel      : 'Task'
                }),

                locationField   = new Ext4.form.TextField({
                    name            : 'Location',
                    fieldLabel      : 'Location'
                })
            ]
    ...
    initComponent : function() {
        ...
        Ext4.apply(this, {
            ...
            items           : [
                ...
                Ext4.applyIf(this.fieldsPanelConfig, {
                    flex        : 1,
                    activeItem  : 0
                })
            ]
    ...
    onSchedulerRender : function() {
        this.render(Ext4.getBody());

        if (this.hideOnBlur) {
            // Hide when clicking outside panel
            this.mon(Ext4.getDoc(), 'mousedown', this.onMouseDown, this);
        }
    },
    ...
    show : function (eventRecord, alignToEl) {
        ...
        if (readOnly !== this.readOnly) {
            Ext4.Array.each(this.query('field'), function(field) {
        ...
        if (this.constrain) {
            this.doConstrain(Ext4.util.Region.getRegion(Ext4.getBody()));
        }
        ...
        if (eventType && this.dynamicForm) {
            ...
            if (!(form instanceof Ext4.form.Panel))              {
    ...
    buildButtons : function() {
        ...
        this.saveButton = new Ext4.Button({
        ...
        this.deleteButton = new Ext4.Button({
        ...
        this.cancelButton = new Ext4.Button({
    ...
    buildDurationFields : function() {
        ...
        this.startDateField = new Ext4.form.field.Date(Ext4.apply({
            width           : 90,
            allowBlank      : false,
        ...
        this.startTimeField = new Ext4.form.field.Time(Ext4.apply({
            width           : 70,
            allowBlank      : false,
        ...
        this.durationField = new Ext4.form.field.Number(Ext4.apply({
        ...
        this.durationLabel = new Ext4.form.Label({
...
Sample usage:
    ...
    plugins : [
        Ext4.create('Sch.plugin.EventTools', {
            items : [
...
*/
Ext4.define('Sch.plugin.EventTools', {
    ...
    */
    ...
    onContainerMouseLeave: function () {
        window.clearTimeout(this.hideTimer);
        this.hideTimer = Ext4.defer(this.hide, this.hideDelay, this);
    },
...
To add this plugin to your scheduler or gantt view:
        ...
        var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
            ...
            plugins         : [
                Ext4.create('Sch.plugin.Pan', { enableVerticalPan : true })
            ]
        });
*/
Ext4.define("Sch.plugin.Pan", {
    extend        : 'Ext4.AbstractPlugin',
    ...
    onMouseDown : function (e, t) {
        ...
        if (e.getTarget('.' + this.view.timeCellCls, 10) && !e.getTarget(this.view.eventSelector)) {
            ...
            Ext4.getBody().on('mousemove', this.onMouseMove, this);
            Ext4.getDoc().on('mouseup', this.onMouseUp, this);
            ...
            if (Ext4.isIE || Ext4.isGecko) {
                Ext4.getBody().on('mouseenter', this.onMouseUp, this);
            }
    ...
    onMouseUp : function (e) {
        Ext4.getBody().un('mousemove', this.onMouseMove, this);
        Ext4.getDoc().un('mouseup', this.onMouseUp, this);

        if (Ext4.isIE || Ext4.isGecko) {
            Ext4.getBody().un('mouseenter', this.onMouseUp, this);
        }
    }
});
...
To add this plugin to scheduler:
        ...
        var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
            ...
            plugins         : [
                Ext4.create('Sch.plugin.SimpleEditor', { dataIndex : 'Title' })
            ]
        });


*/
Ext4.define("Sch.plugin.SimpleEditor", {
    extend          : "Ext4.Editor",
    ...
    mixins          : ['Ext4.AbstractPlugin', 'Sch.mixin.Localizable'],
    lockableScope   : 'top',
    ...
    constructor : function(config) {
        config = config || {};
        config.field = config.field || new Ext4.form.TextField({ selectOnFocus : true });
    ...
    onStartEdit  : function() {
        if (!this.allowBlur) {
            ...
            // http://www.sencha.com/forum/showthread.php?244580-4.1-allowBlur-on-Ext4.Editor-not-working
            Ext4.getBody().on('mousedown', this.onMouseDown, this);
    ...
    onAfterEdit  : function() {
        if (!this.allowBlur) {
            Ext4.getBody().un('mousedown', this.onMouseDown, this);
...
To add this plugin to scheduler:
        ...
        var summaryCol = Ext4.create("Sch.plugin.SummaryColumn", { 
...
*/
Ext4.define('Sch.plugin.SummaryColumn', {
    extend      : "Ext4.grid.column.Column",
    ...
    calculate : function(eventRecords, viewStart, viewEnd){
        ...
        Ext4.each(eventRecords, function(eRec) {
...
To add this plugin to scheduler:
        ...
        var zonesStore = Ext4.create('Ext4.data.Store', {
        ...
        var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
            ...
            plugins         : [
                Ext4.create('Sch.plugin.Zones', { store : zonesStore })
            ]
        });


*/
Ext4.define("Sch.plugin.Zones", {
    extend      : "Sch.feature.AbstractTimeSpan",
    ...
    getElementData : function(viewStart, viewEnd, records, isPrint) {
        ...
        for (var i = 0, l = records.length; i < l; i++) {
            ...
            if (spanStart && spanEnd && Sch.util.Date.intersectSpans(spanStart, spanEnd, viewStart, viewEnd)) {
                ...
                zoneData = Ext4.apply({}, templateData);
    ...
    updateHeaderElement : function(record) {
        var startDate = this.timeAxis.getStart(),
            endDate = this.timeAxis.getEnd(),
            startEl = Ext4.get(this.getHeaderElementId(record, true)),
            endEl   = Ext4.get(this.getHeaderElementId(record, false)),
            startData = this.getZoneHeaderElementData(startDate, endDate, record, true),
            endData   = this.getZoneHeaderElementData(startDate, endDate, record, false);
            
        if (!(startEl && endData) || !(endEl && endData)) {
            Ext4.destroy(startEl, endEl);
            this.renderHeaderElementsInternal([record]);
        } else {
            if (startEl) {
                if (!startData) {
                    Ext4.destroy(startEl);
                } else {
...
To add this plugin to scheduler:
        ...
        var scheduler = Ext4.create('Sch.panel.SchedulerGrid', {
            ...
            plugins         : [
                Ext4.create('Sch.plugin.TimeGap', {
...
*/
Ext4.define("Sch.plugin.TimeGap", {
     ...
     */
    getZoneCls : Ext4.emptyFn,

    init:function(scheduler) {

        this.store = new Ext4.data.JsonStore({
             model : 'Sch.model.Range'
        });
...
@extends Ext4.grid.plugin.CellEditing
...
as usual grid plugin:
    ...
    var gantt = Ext4.create('Gnt.panel.Gantt', {

        plugins             : [
            Ext4.create('Sch.plugin.TreeCellEditing', {
                clicksToEdit: 1
            })
        ],
        ...
...
This class allows us to do 'complex data editing', which is not supported by the regular CellEditing plugin or the Ext4.grid.CellEditor which
...
*/
Ext4.define('Sch.plugin.TreeCellEditing', {
...
//        }
    ...
    showEditor : function(ed, context, value) {
        ...
        if (Ext4.getVersion('extjs').isLessThan('4.2.2.1144')) {
            sm.selectByPosition = Ext4.emptyFn;
        }
...
*/
Ext4.define("Sch.plugin.ResourceZones", {
    extend              : 'Sch.plugin.Zones',
    ...
    */
    ...
    refreshSingle : function(store, record) {
        var el = Ext4.get(this.uniqueCls + '-' + record.internalId);
...
*/
Ext4.define('Sch.widget.ResizePicker', {
    extend          : 'Ext4.Panel',
    ...
    initComponent : function () {
        ...
        me.dockedItems = [
            me.vertical     = new Ext4.slider.Single(Ext4.apply({
                dock        : 'left',
            ...
            me.horizontal   = new Ext4.slider.Single(Ext4.apply({
                dock        : 'top',
    ...
    afterRender : function () {
        ...
        var dd = new Ext4.dd.DD(this.valueHandle);
    ...
    onBodyClick: function (e, t) {
        ...
        this.valueHandle.setLeft(Ext4.Number.constrain(xy[0], 0, this.getAvailableWidth()));
        this.valueHandle.setTop(Ext4.Number.constrain(xy[1], 0, this.getAvailableHeight()));
 ...
 */
Ext4.define('Sch.widget.ExportDialogForm', {
    extend      : 'Ext4.form.Panel',
    requires    : [
        'Ext4.data.Store',
        'Ext4.ProgressBar',
        'Ext4.form.field.ComboBox',
        'Ext4.form.field.Date',
        'Ext4.form.FieldContainer',
    ...
    initComponent : function () {
        ...
        if (Ext4.getVersion('extjs').isLessThan('4.2.1')) {
            if (typeof Ext4.tip !== 'undefined' && Ext4.tip.Tip && Ext4.tip.Tip.prototype.minWidth != 'auto') {
                Ext4.tip.Tip.prototype.minWidth      = 'auto';
            }
        }
    ...
    createFields : function () {
        ...
        me.rangeField = new Ext4.form.field.ComboBox({
        ...
        me.resizerHolder    = new Ext4.form.FieldContainer({
        ...
        // from date
        me.dateFromField = new Ext4.form.field.Date({
            fieldLabel  : cfg.dateRangeFromText,
            baseBodyCls : 'sch-exportdialogform-date',
            name        : 'dateFrom',
            format      : cfg.dateRangeFormat || Ext4.Date.defaultFormat,
            allowBlank  : false,
        ...
        // till date
        me.dateToField = new Ext4.form.field.Date({
        ...
        // date fields holder
        me.datesHolder  = new Ext4.form.FieldContainer({
        ...
        me.showHeaderField = new Ext4.form.field.Checkbox({
            xtype       : 'checkboxfield',
        ...
        me.exportToSingleField = new Ext4.form.field.Checkbox({
            xtype       : 'checkboxfield',
        ...
        me.formatField = new Ext4.form.field.ComboBox({
        ...
        me.orientationField = new Ext4.form.field.ComboBox({
            ...
            afterSubTpl    : new Ext4.XTemplate('<span id="sch-exportdialog-imagePortrait" ' + orientationPortraitCls +
            ...
            listeners      : {
                change : function (field, newValue) {
                    switch (newValue) {
                        case 'landscape':
                            Ext4.fly('sch-exportdialog-imagePortrait').toggleCls('sch-none');
                            Ext4.fly('sch-exportdialog-imageLandscape').toggleCls('sch-none');
                            break;
                        case 'portrait':
                            Ext4.fly('sch-exportdialog-imagePortrait').toggleCls('sch-none');
                            Ext4.fly('sch-exportdialog-imageLandscape').toggleCls('sch-none');
    ...
    createProgressBar : function () {
        return this.progressBar = new Ext4.ProgressBar({
...
/**
 ...
 @private
 @extends Ext4.window.Window

 Widget for export options.

 */
Ext4.define('Sch.widget.ExportDialog', {
    ...
    /**
     * @cfg {Ext4.Component} buttonsPanel Component with buttons controlling export.
     */
    buttonsPanel        : null,
    ...
    /**
     * @cfg {Ext4.Component} progressBar Progress bar component.
     */
    progressBar         : null,
    ...
    constructor : function (config) {
        Ext4.apply(this, config.exportDialogConfig);
...
*/
Ext4.define("Sch.feature.ColumnLines", {
    extend : 'Sch.plugin.Lines',
    ...
    getData : function() {
        ...
        if (panel.isHorizontal()) {
            this.timeAxisViewModel.forEachMainInterval(function(start, end, i) {
                if (i > 0) {
...
*/
Ext4.define("Sch.mixin.AbstractTimelineView", {
    requires: [
        'Sch.data.TimeAxis',
        'Sch.view.Horizontal'
    ],
...
//    barMargin           : null,
    ...
    // Accessor to the Ext4.Element for this view
    el                  : null,    

    _initializeTimelineView         : function() { 
        if (this.horizontalViewClass) {
            this.horizontal = Ext4.create(this.horizontalViewClass, { view : this });
        }

        if (this.verticalViewClass) {
            this.vertical = Ext4.create(this.verticalViewClass, { view : this });
        }
    ...
    */
    getFormattedDate: function (date) {
        return Ext4.Date.format(date, this.getDisplayDateFormat());
    },
    ...
    */
    getFormattedEndDate: function (endDate, startDate) {
        ...
        return Ext4.Date.format(endDate, format);
    },
    ...
    */ 
    fitColumns: function (preventRefresh) { // TODO test
        if (this.orientation === 'horizontal') {
            ...
            var w = Math.floor((this.panel.getWidth() - Ext4.getScrollbarSize().width - 1) / this.headerCt.getColumnCount());
    ...
    * @return {Ext4.Element} The Ext4.Element representing the event record
    */
    getElementFromEventRecord: function (record) {
        return Ext4.get(this.eventPrefix + record.internalId);
    },
    ...
    *  @param {Ext4.EventObject} e, the EventObject instance
    ...
    */
    ...
    /**
    * Gets the Ext4.util.Region represented by the schedule and optionally only for a single resource. The view will ask the scheduler for 
    ...
    */
    ...
    getDateConstraints : Ext4.emptyFn
});


Ext4.apply(Sch, {
    /*PKGVERSION*/VERSION : '2.2.16'
});
/**
@class Sch.mixin.TimelineView

A base mixin for {@link Ext4.view.View} classes, giving to the consuming view the "time line" functionality.
This means that the view will be capable to display a list of "events", ordered on the {@link Sch.data.TimeAxis time axis}.
...
*/
Ext4.define("Sch.mixin.TimelineView", {
    ...
    */
        ...
        return {
            ...
            processUIEvent: function(e){
                ...
                if (eventBarNode && type in map) {
                    ...
                    preventViewEvent = !(this.getSelectionModel() instanceof Ext4.selection.RowModel);
                }
    ...
    onEventMouseOver: function (e, t) {
        if (t !== this.lastItem) {
            ...
            Ext4.fly(t).addCls(this.overScheduledEventClass);
    ...
    onEventMouseOut: function (e, t) {
        if (this.lastItem) {
            if (!e.within(this.lastItem, true, true)) {
                Ext4.fly(this.lastItem).removeCls(this.overScheduledEventClass);
    ...
    highlightItem: function(item) {
        if (item) {
            ...
            Ext4.fly(item).addCls(me.overItemCls);
        }
    },
    ...
    setupTooltip: function () {
        var me = this,
            tipCfg = Ext4.apply({
                ...
                show : function() {
                    Ext4.ToolTip.prototype.show.apply(this, arguments);
                    ...
                    if (this.triggerElement && me.getOrientation() === 'horizontal') {
                        ...
                        this.setY(Ext4.fly(this.triggerElement).getY()-this.getHeight()-10);
                    }
                }
            }, me.tipCfg);

        me.tip = new Ext4.ToolTip(tipCfg);
    ...
    */
    ...
    refreshKeepingScroll : function() {
        ...
        if (this.scrollState.left !== 0 || this.scrollState.top !== 0) {
    ...
    getTableRegion: function () {
        var tableEl = this.el.down('.' + Ext4.baseCSSPrefix + 'grid-table');
...
Ext4.define('Sch.view.TimelineGridView', {
    extend                  : 'Ext4.grid.View',
    ...
    constructor : function (config) {
        ...
        if (this.infiniteScroll) this.on('viewready', this.setupInfiniteScroll, this, { single : true });
    },
    ...
    setupInfiniteScroll : function () {
        ...
        this.panel.ownerCt.on('zoomchange', this.onSelfResize, this);
    },
    ...
    refresh : function () {
        ...
        // if infinite scroll enabled we need to set left scroll position to
        // the saved date after each refresh
        if (this.infiniteScroll) {
            // if we have no saved date then it's the first refresh
            if (this.deferInitialRefresh && !this.getScrollDate()) {
                this.onSelfResize();
            } else {
                // scroll to saved date
                this.restoreDateScroll();
            }
        }
    },


    onSelfResize : function () {
        this.shiftToDate(this.getScrollDate() || this.timeAxis.getStart());
    },


    setScrollDate : function (scrollDate) {
        this._scrollToDate  = scrollDate;
    },


    getScrollDate : function () {
        return this._scrollToDate;
    },


    restoreDateScroll : function () {
        if (this._scrollToDate) {
            this.scrollToDate(this._scrollToDate);
        }
    },


    scrollToDate : function (toDate) {
        this.setScrollDate(toDate);
    ...
    shiftToDate : function (toDate) {
        var viewWidth           = this.getWidth();
        var tickWidth           = this.timeAxisViewModel.getTickWidth();

        var timeAxis            = this.timeAxis,
            increment           = timeAxis.increment || 1;

        var DATE                = Sch.util.Date;

        var bufferedTicks       = Math.ceil(viewWidth * this.bufferCoef / tickWidth);

        var realStartDate       = DATE.add(toDate, timeAxis.unit, -bufferedTicks * increment);
        var realEndDate         = DATE.add(toDate, timeAxis.unit, (viewWidth/tickWidth + bufferedTicks) * increment);

        // we set scroll date here since it will be required during timeAxis.setTimeSpan() call
        this.setScrollDate(toDate);

        timeAxis.setTimeSpan(realStartDate, realEndDate);

        // scroll to date (and remember the scroll date)
        this.scrollToDate(toDate);
    },
...
A mixin for {@link Ext4.view.View} classes, providing "scheduling" functionality to the consuming view. A consuming class
should have already consumed the {@link Sch.mixin.TimelineView} mixin.

Generally, should not be used directly, if you need to subclass the view, subclass the {@link Sch.view.SchedulerGridView} instead.

*/
Ext4.define('Sch.mixin.AbstractSchedulerView', {
    requires: [
    ...
    */
    ...
    eventTpl: [
        '<tpl for=".">',
            '<div unselectable="on" id="{{evt-prefix}}{id}" style="right:{right}px;left:{left}px;top:{top}px;height:{height}px;width:{width}px;{style}" class="sch-event ' + Ext4.baseCSSPrefix + 'unselectable {internalCls} {cls}">',
    ...
    _initializeSchedulerView        : function() {
        ...
        if (Sch.eventlayout.Horizontal) {
            this.eventLayout.horizontal = new Sch.eventlayout.Horizontal({ view : this, timeAxisViewModel : this.timeAxisViewModel });
        }

        if (Sch.eventlayout.Vertical) {
            this.eventLayout.vertical = new Sch.eventlayout.Vertical({ view : this, timeAxisViewModel : this.timeAxisViewModel });
        }
    ...
    generateTplData: function (event, resourceRecord, resourceIndex) {
        ...
        if (this.eventRenderer) {
            ...
            if (Ext4.isObject(value) && this.eventBodyTemplate) {
    ...
    * @param {HTMLElement/Ext4.Element} el The DOM node or Ext Element to lookup
    * @return {Sch.model.Event} The event record
    */
    resolveEventRecord: function (el) {
        ...
        if (!(Ext4.fly(el).hasCls(this.eventCls))) {
            el = Ext4.fly(el).up(this.eventSelector);
        }
    ...
    * @return {Ext4.util.MixedCollection} The collection of events
    */
    ...
    */
    ...
    bindEventStore: function (eventStore, initial) {
        ...
        if (eventStore) {
            eventStore = Ext4.data.StoreManager.lookup(eventStore);
    ...
    */
    ...
    scrollEventIntoView: function (eventRec, highlight, animate, callback, scope) {
        ...
        if (Ext4.data.TreeStore && this.resourceStore instanceof Ext4.data.TreeStore) {
...
*/
Ext4.define('Sch.mixin.SchedulerView', {
    ...
    */
    ...
    _initializeSchedulerView : function() {
        ...
        this.addEvents(
             ...
             * @param {Ext4.Element} element The proxy element being resized
             */
            'eventpartialresize',
        ...
        if (Ext4.isArray(me.eventTpl)) {
            var clone       = Ext4.Array.clone(me.eventTpl),
            ...
            if (eventResizeHandles === 'start' || eventResizeHandles === 'both') {
                clone.splice(2, 0, Ext4.String.format(resizeTpl, 'start'));
            }
            if (eventResizeHandles === 'end' || eventResizeHandles === 'both') {
                clone.splice(2, 0, Ext4.String.format(resizeTpl, 'end'));
            }

            me.eventTpl     = new Ext4.XTemplate(clone.join('').replace('{{evt-prefix}}', this.eventPrefix));
        }
    },
    ...
    _afterRender: function () {
        ...
        if (resizer) {
            resizer.doResize = Ext4.Function.createSequence(resizer.doResize, this.afterHeaderResized, this);
        }
    },
    ...
    configureFunctionality: function () {
        ...
        if (this.eventResizeHandles !== 'none' && Sch.feature.ResizeZone) {
            this.resizePlug = new Sch.feature.ResizeZone(Ext4.applyIf({
                schedulerView: this,
        ...
        if (this.enableDragCreation !== false && Sch.feature.DragCreator) {
            this.dragCreator = new Sch.feature.DragCreator(Ext4.applyIf({
                schedulerView: this,
...
*/
Ext4.define("Sch.view.SchedulerGridView", {
...
Ext4.define('Sch.mixin.Zoomable', {
    ...
    getMilliSecondsPerPixelForZoomLevel : function (level) {
        ...
        return Math.round(
            ...
            // `actualWidth` is a width after view adjustments we use it if available to return the precise index value
            (level.actualWidth || level.width)
        );
    },
    ...
    */
    ...
    zoomToLevel: function (level, span) {
        level                       = Ext4.Number.constrain(level, this.minZoomLevel, this.maxZoomLevel);
        ...
        presetCopy[ isVertical ? 'timeRowHeight' : 'timeColumnWidth' ] = nextZoomLevel.width;
    ...
    zoomToSpan : function (span, config) {
        if (span.start && span.end && span.start < span.end) {
            ...
            while (level >= 0 && level <= this.zoomLevels.length - 1) {
                ...
                // get its preset copy
                preset      = Ext4.clone(Sch.preset.Manager.getPreset(zoomLevel.preset));
    ...
    *
    * @param {Int} levels (optional) amount of levels to zoom in
    *
    ...
    * Otheriwse a value of `1` will be used.
    *
    * @param {Int} levels (optional) amount of levels to zoom out
    *
    * @return {Int} currentLevel New zoom level of the panel or null if level hasn't changed.
    */
...
Ext4.define('Sch.mixin.AbstractTimelinePanel', {
    requires: [
        'Sch.data.TimeAxis',
    ...
    */
    ...
    deregisterRenderer : function(fn, scope) {
        Ext4.each(this.renderers, function(rend, i) {
            if (fn === rend) {
                Ext4.Array.removeAt(this.renderers, i);
                return false;
            }
        });
    }
...
/**
 ...
 A base mixing for {@link Ext4.panel.Panel} classes, giving to the consuming panel the "time line" functionality.
...
Ext4.define('Sch.mixin.TimelinePanel', {
    ...
    */
    ...
    /**
    * @cfg {Object} tipCfg
    * The {@link Ext4.Tooltip} config object used to configure a tooltip (only applicable if tooltipTpl is set).
    */
    tipCfg: {
        cls: 'sch-tip',
    ...
    inheritables: function() {
        ...
        return {
            ...
            initComponent: function () {
                ...
                if (Ext4.versions.extjs.isGreaterThanOrEqual("4.2.1")) {
                ...
                Ext4.apply(this.normalViewConfig, {
                    id                      : id + '-timelineview',
                ...
                if (!this.startDate) {
                    ...
                    if (Ext4.data.TreeStore && store instanceof Ext4.data.TreeStore ? store.getRootNode().childNodes.length : store.getCount()) {
                ...
                if (columnLines) {
                    this.columnLinesFeature = new Sch.feature.ColumnLines(Ext4.isObject(columnLines) ? columnLines : undefined);
    ...
    bindAutoTimeSpanListeners : function () {
        ...
        store.on('load', this.applyStartEndDatesFromStore, this);

        if (Ext4.data.TreeStore && store instanceof Ext4.data.TreeStore) {
            store.on('rootchange', this.applyStartEndDatesFromStore, this);
            store.tree.on('append', this.applyStartEndDatesAfterTreeAppend, this);

        } else {
            store.on('add', this.applyStartEndDatesFromStore, this);
        }
    },
    ...
    */
    ...
    configureColumns : function() {
        ...
        // The 'columns' config can also be a config object for Ext4.grid.header.Container
        if (columns.items) {
        ...
        Ext4.Array.each(columns, function (column) {
            if (column.position === 'right') {
                if (!Ext4.isNumber(column.width)) {
                    Ext4.Error.raise('"Right" columns must have a fixed width');
                }
                column.locked = false;
        ...
        Ext4.Array.erase(columns, 0, columns.length);
        Ext4.Array.insert(columns, 0, lockedColumns.concat(
            {
        ...
        this.horizontalColumns = Ext4.Array.clone(columns);
        ...
        if (this.orientation === 'vertical') {
            ...
            this.on('beforerender', function () {
                this.normalGrid.headerCt.add(this.createResourceColumns(this.resourceColumnWidth || this.timeAxisViewModel.resourceColumnWidth));
            }, this);
        }
    },
    ...
    if (Ext4.versions.extjs.isLessThan(MIN_EXT_VERSION)) {
...
Ext4.define('Sch.mixin.AbstractSchedulerPanel', {
    ...
    *
    *      return Ext4.Date.format(eventRec.getStartDate(), 'Y-m-d') + ': ' + eventRec.getName();
    *  }
    *</pre>
    ...
    */
    ...
    /**
     * @cfg {Sch.data.EventStore} eventStore The {@link Ext4.data.Store} holding the events to be rendered into the scheduler (required).
     * @required
    */
    eventStore: null,

    /**
    * @cfg {Sch.data.ResourceStore} resourceStore The {@link Ext4.data.Store} holding the resources to be rendered into the scheduler (required).
    */
    resourceStore: null,
    ...
    /**
    * @cfg {Ext4.Template} eventTpl The wrapping template used to renderer your events in the scheduler. Normally you should not override this,
    ...
    */
    ...
    /**
    * @cfg {String/Ext4.Template} eventBodyTemplate The template used to generate the markup of your events in the scheduler. To 'populate' the eventBodyTemplate with data, use the {@link eventRenderer} method 
    */

    /**
    *  @cfg {Object} timeAxisColumnCfg A {@link Ext4.grid.column.Column} config used to configure the time axis column in vertical orientation.
    */
    ...
    initStores : function() {
        ...
        if (!resourceStore) {
            Ext4.Error.raise("You must specify a resourceStore config");
        }

        if (!this.eventStore) {
            Ext4.Error.raise("You must specify an eventStore config");
        }

        // Set "store" for the grid panel API
        this.store          = Ext4.StoreManager.lookup(resourceStore);
        this.resourceStore  = this.store;
        this.eventStore     = Ext4.StoreManager.lookup(this.eventStore);

        if (!this.eventStore.isEventStore) {
            Ext4.Error.raise("Your eventStore should be a subclass of Sch.data.EventStore (or consume the EventStore mixin)");
        }
    ...
    _initializeSchedulerPanel : function() {
        ...
        if (this.eventBodyTemplate && Ext4.isString(this.eventBodyTemplate)) {
            this.eventBodyTemplate = new Ext4.XTemplate(this.eventBodyTemplate);
        }
    },
    ...
    */
    ...
    createResourceColumns : function (colWidth) {
        ...
        this.resourceStore.each(function (resource) {
            newItems.push(
                Ext4.create(me.resourceColumnClass, {
...
/**
 ...
 A mixin for {@link Ext4.panel.Panel} classes, providing "scheduling" functionality to the consuming panel.
 ...
 */
Ext4.define('Sch.mixin.SchedulerPanel', {
        ...
        return {
            ...
            initComponent : function () {
                ...
                if (this.resourceZones) {
                    var resourceZoneStore = Ext4.StoreManager.lookup(this.resourceZones);
                    resourceZoneStore.setResourceStore(this.resourceStore);

                    this.resourceZonesPlug = new Sch.plugin.ResourceZones(Ext4.apply({
            ...
            afterRender : function (){
                ...
                this.getSchedulingView().on({
                    ...
                    eventdrop           : this.doResumeLayouts,
    ...
    enableRowHeightInjection : function (lockedView, schedulingView) {
        var cellTpl = new Ext4.XTemplate(
            '{%',
    ...
    getEventSelectionModel : function () {
        ...
        Ext4.applyIf(eventSelModel, {
            allowDeselect : this.allowDeselect,
            mode          : mode
        });

        if (!eventSelModel.events) {
            eventSelModel = this.eventSelModel = Ext4.create('selection.' + this.eventSelModelType, eventSelModel);
        }
    ...
    refreshResourceColumns : function () {
        ...
        headerCt.add(this.createResourceColumns(this.resourceColumnWidth || this.timeAxisViewModel.resourceColumnWidth));
    ...
    onScheduleRowMouseDown : function(view, resource) {
        ...
        if (this.getOrientation() === 'horizontal' && Ext4.selection.RowModel && lockedSelModel instanceof Ext4.selection.RowModel) {
...
Ext4.define("Sch.mixin.FilterableTreeView", {
    ...
    initTreeFiltering : function () {
        var doInit  = function () {
            ...
            if (treeStore instanceof Ext4.data.NodeStore) treeStore  = this.up('tablepanel[lockable=true]').store;
...
*/
Ext4.define("Sch.panel.TimelineGridPanel", {
...
@extends Ext4.tree.Panel
@mixin Sch.mixin.TimelinePanel

Internal class.

*/
Ext4.define("Sch.panel.TimelineTreePanel", {
...
A scheduler panel based on the {@link Ext4.grid.Panel} class which allows you to visualize and manage "resources" and their scheduled "events".
...
*/
Ext4.define("Sch.panel.SchedulerGrid", {
...
- in your columns definition, you must include a column which will contain the tree itself (the `treecolumn` xtype):
        ...
        var schedulerTree = Ext4.create('Sch.panel.SchedulerTree', {
            columns: [
                {
                    xtype       : 'treecolumn',
...
*/
Ext4.define("Sch.panel.SchedulerTree", {
    ...
    setOrientation : function (orientation) {
        if (orientation == 'vertical') {
            Ext4.Error.raise("Sch.panel.SchedulerTree does not support vertical orientation");
        }
    },
